...
const discordIdSchema = z.string().regex(/^\d{15,22}$/, "Invalid Discord ID");
...
app.post("/api/v1/subjects", async (req, reply) => {
  const body = z.object({
    ...
    confidence: z.enum(["UNVERIFIED","LOW","MEDIUM","HIGH","CONFIRMED"]).default("UNVERIFIED"),
...
app.post("/api/v1/servers", async (req, reply) => {
  ...
  const server = await prisma.discordServer.create({ data: body });
  return reply.code(201).send(server);
});

app.get("/api/v1/users/:discordId/evidence", async (req, reply) => {
  const { discordId } = z.object({ discordId: discordIdSchema }).parse(req.params);
  ...
  if (!user) return reply.code(404).send({ found: false, discordId });
...
app.post("/api/v1/users/:discordId/evidence", async (req, reply) => {
  const { discordId } = z.object({ discordId: discordIdSchema }).parse(req.params);
  ...
  const body = z.object({
    ...
    confidence: z.enum(["UNVERIFIED","LOW","MEDIUM","HIGH","CONFIRMED"]).default("UNVERIFIED"),
    roleNames: z.array(z.string().max(100)).default([]),
  ...
  if (body.serverDiscordId) {
    const server = await prisma.discordServer.findUnique({
      where: { discordId: body.serverDiscordId }
    });
    if (!server) {
      return reply.code(400).send({
        error: "Unknown serverDiscordId. Add the Discord server to /api/v1/servers first."
      });
    }
    serverId = server.id;
  }
...
app.delete("/api/v1/evidence/:id", async (req, reply) => {
